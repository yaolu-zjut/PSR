...
class ZeroNas:
    def __init__(self, dataloader, indicator, criterion=nn.CrossEntropyLoss(), num_batch=1):
    ...
    def get_score(self, model):
        ...
        model.train()
        scores = dict()
        for i, (signals, target) in enumerate(self.dataloader):
            # print("批次", i + 1, "信号大小:", signals.size())
            signals = signals.cuda()
            target = target.cuda()
            for k, score_func in self.indicator.items():
                if k in scores.keys():
                    scores[k].append(score_func(model, signals, target, self.criterion))
                else:
                    scores[k] = [score_func(model, signals, target, self.criterion)]
